...
def move_totem_to(totem: List[CoordinatePair], pos: CoordinatePair) -> List[CoordinatePair]:
...
class SearchNode:
    ...
    def __init__(self, shapes_left: List[Totem], max_x: int, max_y: int,
                 used: Set[CoordinatePair], holes: Set[CoordinatePair],
    ...
    def with_totem(self, totem: TotemAnswer) -> 'SearchNode':
        ...
        if max_x > self.max_x:
            holes |= {(x, y)
                      for x in range(self.max_x, max_x + 1)
        ...
        if max_y > self.max_y:
            holes |= {(x, y)
                      ...
                      for y in range(self.max_y, max_y + 1)}
        max_y = max(self.max_y, max_y)
        return SearchNode(shapes_left=new_shapes_left, max_x=max_x, max_y=max_y,
                          used=self.used & set(totem.coordinates), holes=holes - set(totem.coordinates),
                ...
                if not self.totems:
                    ...
                    for hole in self.holes:
                        ...
                        if self.shape_fits(new_variant):
                            yield self.with_totem(TotemAnswer(shape=shape, coordinates=new_variant))
...
def solve(shapes: List[Totem]) -> List[TotemAnswer]:
    start = SearchNode(shapes_left=shapes, max_x=0, max_y=0, used=set(), holes=set(), totems=[])
    path = astar.find_path(start=start, goal=None,
