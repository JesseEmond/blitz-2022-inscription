...
SHAPE_VARIANT_DIMS = {
    shape: [(max(x for x, _ in variant) + 1,
             max(y for _, y in variant) + 1)
...
def variants_with_dims(shape: Totem) -> List[Tuple[List[CoordinatePair], CoordinatePair]]:
    return [(SHAPE_VARIANTS[shape][i], SHAPE_VARIANT_DIMS[shape][i]) for i in range(len(SHAPE_VARIANTS[shape]))]